Shader "Unlit/DepthGradientConfidenceBlend"
{
    Properties
    {
        _MainTex ("Main Texture", 2D) = "black" {}
        _ConfidenceTex ("Confidence Texture", 2D) = "black" {}
        _ConfidenceThreshold ("Confidence Threshold", Range(-1,0)) = -0.5 // -1 is low confidence, 0 is full confidence
    ...
    SubShader
    {
        ...
        HLSLPROGRAM
            ...
            DECLARE_TEXTURE2D_FLOAT(_ConfidenceTex);
            DECLARE_SAMPLER_FLOAT(sampler_ConfidenceTex);

            half _ConfidenceThreshold;
            half _MinDistance;
            half _MaxDistance;
            ...
            fragment_output frag (v2f i)
            {
                ...
                // Sample the confidence texture.
                float confidence = SAMPLE_TEXTURE2D(_ConfidenceTex, sampler_ConfidenceTex, uv).r;
                if (confidence < _ConfidenceThreshold)
                {
                    // Low confidence, return black.
                    // TODO: Maybe blur with confident neighbors?
                    fragment_output o;
                    o.color = half4(0.0h, 0.0h, 0.0h, 1.0h);
                    return o;
                }
